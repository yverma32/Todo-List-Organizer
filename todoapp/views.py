    ...
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                return render(request, 'todoapp/signupuser.html', {'form':UserCreationForm(), 'error':'Username already exists, please choose another username'})

        else:
            # Tell the user that pass didn't match
...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'todoapp/loginuser.html', {'form':AuthenticationForm(), 'error':'Username or password does not match'})
        else:
            login(request, user)
            return redirect('currenttodos')
@login_required
...
def viewtodo(request, todo_pk):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, 'todoapp/viewtodo.html', {'todo':todo, 'form':form, 'error':'Something is wrong'})
